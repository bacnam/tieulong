...
public static String TABLE_NAME = "GoldChargeLog";
...
return "INSERT INTO GoldChargeLog(`server_id`, `timestamp`, `date_time`, `pid`, `vip_level`, `level`, `reason`, `num`, `cur_remainder`, `pre_value`, `type`)values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
}

public static String getCreateTableSQL() {
String sql = "CREATE TABLE IF NOT EXISTS `GoldChargeLog` (`id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',`server_id` int(11) NOT NULL DEFAULT '0' COMMENT '服务器ID',`timestamp` int(11) NOT NULL DEFAULT '0' COMMENT '日志时间(时间戳)',`date_time` varchar(500) NOT NULL DEFAULT '20160801' COMMENT '日志时间(yyyymmdd)',`pid` bigint(20) NOT NULL DEFAULT '0' COMMENT '玩家pid',`vip_level` int(11) NOT NULL DEFAULT '0' COMMENT 'VIP等级',`level` int(11) NOT NULL DEFAULT '0' COMMENT '队伍等级',`reason` int(11) NOT NULL DEFAULT '0' COMMENT '产生原因类型',`num` int(11) NOT NULL DEFAULT '0' COMMENT '数量',`cur_remainder` int(11) NOT NULL DEFAULT '0' COMMENT '当前剩余',`pre_value` int(11) NOT NULL DEFAULT '0' COMMENT '前值',`type` int(11) NOT NULL DEFAULT '0' COMMENT '类型(1:获得,2:消耗)',PRIMARY KEY (`id`)) COMMENT='Coin Charge 金币消费日志' DEFAULT CHARSET=utf8";
