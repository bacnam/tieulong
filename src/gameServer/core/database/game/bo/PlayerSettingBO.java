/*     */ package core.database.game.bo;
/*     */ 
/*     */ import com.zhonglian.server.common.db.BaseBO;
/*     */ import com.zhonglian.server.common.db.annotation.DataBaseField;
/*     */ import core.server.ServerConfig;
/*     */ import java.sql.ResultSet;
/*     */ import java.util.ArrayList;
/*     */ import java.util.List;
/*     */ 
/*     */ 
/*     */ public class PlayerSettingBO
/*     */   extends BaseBO
/*     */ {
/*     */   @DataBaseField(type = "bigint(20)", fieldname = "id", comment = "数据库表唯一键值")
/*     */   private long id;
/*     */   @DataBaseField(type = "bigint(20)", fieldname = "pid", comment = "*所属玩家ID")
/*     */   private long pid;
/*     */   @DataBaseField(type = "int(11)", fieldname = "type", comment = "设置类型,对应枚举类型ProtoEnum.SettingType")
/*     */   private int type;
/*     */   @DataBaseField(type = "int(11)", fieldname = "value", comment = "设置对应值")
/*     */   private int value;
/*     */   @DataBaseField(type = "int(11)", fieldname = "lastChangeTime", comment = "最近一次修改时间,单位秒")
/*     */   private int lastChangeTime;
/*     */   
/*     */   public PlayerSettingBO() {
/*  26 */     this.id = 0L;
/*  27 */     this.pid = 0L;
/*  28 */     this.type = 0;
/*  29 */     this.value = 0;
/*  30 */     this.lastChangeTime = 0;
/*     */   }
/*     */   
/*     */   public PlayerSettingBO(ResultSet rs) throws Exception {
/*  34 */     this.id = rs.getLong(1);
/*  35 */     this.pid = rs.getLong(2);
/*  36 */     this.type = rs.getInt(3);
/*  37 */     this.value = rs.getInt(4);
/*  38 */     this.lastChangeTime = rs.getInt(5);
/*     */   }
/*     */ 
/*     */ 
/*     */   
/*     */   public void getFromResultSet(ResultSet rs, List<PlayerSettingBO> list) throws Exception {
/*  44 */     list.add(new PlayerSettingBO(rs));
/*     */   }
/*     */ 
/*     */   
/*     */   public long getAsynTaskTag() {
/*  49 */     return 0L;
/*     */   }
/*     */ 
/*     */   
/*     */   public String getItemsName() {
/*  54 */     return "`id`, `pid`, `type`, `value`, `lastChangeTime`";
/*     */   }
/*     */ 
/*     */   
/*     */   public String getTableName() {
/*  59 */     return "`playerSetting`";
/*     */   }
/*     */ 
/*     */   
/*     */   public String getItemsValue() {
/*  64 */     StringBuilder strBuf = new StringBuilder();
/*  65 */     strBuf.append("'").append(this.id).append("', ");
/*  66 */     strBuf.append("'").append(this.pid).append("', ");
/*  67 */     strBuf.append("'").append(this.type).append("', ");
/*  68 */     strBuf.append("'").append(this.value).append("', ");
/*  69 */     strBuf.append("'").append(this.lastChangeTime).append("', ");
/*  70 */     strBuf.deleteCharAt(strBuf.length() - 2);
/*  71 */     return strBuf.toString();
/*     */   }
/*     */ 
/*     */   
/*     */   public ArrayList<byte[]> getInsertValueBytes() {
/*  76 */     ArrayList<byte[]> ret = (ArrayList)new ArrayList<>();
/*  77 */     return ret;
/*     */   }
/*     */ 
/*     */   
/*     */   public void setId(long iID) {
/*  82 */     this.id = iID;
/*     */   }
/*     */ 
/*     */   
/*     */   public long getId() {
/*  87 */     return this.id;
/*     */   }
/*     */   
/*     */   public long getPid() {
/*  91 */     return this.pid;
/*     */   } public void setPid(long pid) {
/*  93 */     if (pid == this.pid)
/*     */       return; 
/*  95 */     this.pid = pid;
/*     */   }
/*     */   public void savePid(long pid) {
/*  98 */     if (pid == this.pid)
/*     */       return; 
/* 100 */     this.pid = pid;
/* 101 */     saveField("pid", Long.valueOf(pid));
/*     */   }
/*     */   
/*     */   public int getType() {
/* 105 */     return this.type;
/*     */   } public void setType(int type) {
/* 107 */     if (type == this.type)
/*     */       return; 
/* 109 */     this.type = type;
/*     */   }
/*     */   public void saveType(int type) {
/* 112 */     if (type == this.type)
/*     */       return; 
/* 114 */     this.type = type;
/* 115 */     saveField("type", Integer.valueOf(type));
/*     */   }
/*     */   
/*     */   public int getValue() {
/* 119 */     return this.value;
/*     */   } public void setValue(int value) {
/* 121 */     if (value == this.value)
/*     */       return; 
/* 123 */     this.value = value;
/*     */   }
/*     */   public void saveValue(int value) {
/* 126 */     if (value == this.value)
/*     */       return; 
/* 128 */     this.value = value;
/* 129 */     saveField("value", Integer.valueOf(value));
/*     */   }
/*     */   
/*     */   public int getLastChangeTime() {
/* 133 */     return this.lastChangeTime;
/*     */   } public void setLastChangeTime(int lastChangeTime) {
/* 135 */     if (lastChangeTime == this.lastChangeTime)
/*     */       return; 
/* 137 */     this.lastChangeTime = lastChangeTime;
/*     */   }
/*     */   public void saveLastChangeTime(int lastChangeTime) {
/* 140 */     if (lastChangeTime == this.lastChangeTime)
/*     */       return; 
/* 142 */     this.lastChangeTime = lastChangeTime;
/* 143 */     saveField("lastChangeTime", Integer.valueOf(lastChangeTime));
/*     */   }
/*     */ 
/*     */ 
/*     */ 
/*     */   
/*     */   protected String getUpdateKeyValue() {
/* 150 */     StringBuilder sBuilder = new StringBuilder();
/* 151 */     sBuilder.append(" `pid` = '").append(this.pid).append("',");
/* 152 */     sBuilder.append(" `type` = '").append(this.type).append("',");
/* 153 */     sBuilder.append(" `value` = '").append(this.value).append("',");
/* 154 */     sBuilder.append(" `lastChangeTime` = '").append(this.lastChangeTime).append("',");
/* 155 */     sBuilder.deleteCharAt(sBuilder.length() - 1);
/* 156 */     return sBuilder.toString();
/*     */   }
/*     */   
/*     */   public static String getSql_TableCreate() {
/* 160 */     String sql = "CREATE TABLE IF NOT EXISTS `playerSetting` (`id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',`pid` bigint(20) NOT NULL DEFAULT '0' COMMENT '*所属玩家ID',`type` int(11) NOT NULL DEFAULT '0' COMMENT '设置类型,对应枚举类型ProtoEnum.SettingType',`value` int(11) NOT NULL DEFAULT '0' COMMENT '设置对应值',`lastChangeTime` int(11) NOT NULL DEFAULT '0' COMMENT '最近一次修改时间,单位秒',KEY `pid` (`pid`),PRIMARY KEY (`id`)) COMMENT='玩家设置表,按表的id字段排序'  DEFAULT CHARSET=utf8 AUTO_INCREMENT=" + (
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */       
/* 168 */       ServerConfig.getInitialID() + 1L);
/* 169 */     return sql;
/*     */   }
/*     */ }


/* Location:              /Users/bacnam/Projects/TieuLongProject/gameserver/gameServer.jar!/core/database/game/bo/PlayerSettingBO.class
 * Java compiler version: 8 (52.0)
 * JD-Core Version:       1.1.3
 */