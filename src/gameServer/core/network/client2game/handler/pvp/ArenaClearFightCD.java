...
int times = recorder.getValue(ConstEnum.DailyRefresh.ArenaResetFightCD);
RefCrystalPrice prize = RefCrystalPrice.getPrize(times);
if (!currency.check(PrizeType.Crystal, prize.ArenaResetFightCD)) {
throw new WSException(ErrorCode.NotEnough_Crystal, "玩家第%s次清除需要钻石%s", new Object[] { Integer.valueOf(times), Integer.valueOf(prize.ArenaResetFightCD) });
}
currency.consume(PrizeType.Crystal, prize.ArenaResetFightCD, ItemFlow.ArenaClearFightCD);
recorder.addValue(ConstEnum.DailyRefresh.ArenaResetFightCD);

Competitor competitor = manager.getOrCreate(player.getPid());
competitor.setFightCD(0);
request.response(Integer.valueOf(recorder.getValue(ConstEnum.DailyRefresh.ArenaResetFightCD)));
}
}
