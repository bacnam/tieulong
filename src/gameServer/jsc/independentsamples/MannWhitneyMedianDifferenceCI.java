...
long l = (i * j);
if (l <= 400L) { paramInt = 1; }
else if (l <= 10000L) { paramInt = 2; }
else { paramInt = 3; }
...
while (l < mannWhitneyU.getMaxValue() && d1 < d) { l++; d1 += mannWhitneyU.pdf(l); }
...
this.xA = paramArrayOfdouble1; this.xB = paramArrayOfdouble2;
} 
}
...
double d3 = 0.0D; int i;
for (i = k; i <= m; ) { d3 += param1ArrayOfdouble[i]; i++; }
double d4 = (param1Int - 2 * j);
this.zbar = d3 / d4;

d3 = 0.0D;
for (i = k; i <= m; ) { d3 += (param1ArrayOfdouble[i] - this.zbar) * (param1ArrayOfdouble[i] - this.zbar); i++; }
...
double getMean() { return this.zbar; } double getVariance() {
return this.varzb;
}
}
...
Normal normal2 = new Normal(1.0D, 1.0D); byte b;
for (b = 0; b < c; ) { arrayOfDouble1[b] = normal1.random(); b++; }
for (b = 0; b < i; ) { arrayOfDouble2[b] = normal2.random(); b++; }
