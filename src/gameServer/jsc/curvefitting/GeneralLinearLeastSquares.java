...
for (byte b = 0; b < this.n; ) { this.weight[b] = 1.0D; b++; }
...
double[] arrayOfDouble1 = new double[i]; byte b;
for (b = 0; b < i; b++) {

double d = 0.0D;
if (paramArrayOfdouble1[b] != 0.0D) {

for (byte b1 = 0; b1 < j; ) { d += paramMatrix1.get(b1, b) * paramArrayOfdouble2[b1]; b1++; }
...
double d = 0.0D;
for (byte b1 = 0; b1 < i; ) { d += paramMatrix2.get(b, b1) * arrayOfDouble1[b1]; b1++; }
arrayOfDouble2[b] = d;
} 
...
Matrix matrix1 = new Matrix(this.n, this.ma); byte b2;
for (b2 = 0; b2 < this.n; b2++) {

double[] arrayOfDouble = this.fv.function(paramArrayOfdouble1[b2]);
double d = this.weight[b2];
for (byte b = 0; b < this.ma; ) { matrix1.set(b2, b, arrayOfDouble[b] * d); b++; }
...
double[] arrayOfDouble = this.fv.function(paramArrayOfdouble1[b2]); double d5;
for (d5 = 0.0D, b1 = 0; b1 < this.ma; ) { d5 += this.a[b1] * arrayOfDouble[b1]; b1++; }
...
double[] arrayOfDouble = new double[i]; byte b;
for (b = 0; b < i; b++) {
...
byte b2; double d;
for (d = 0.0D, b2 = 0; b2 < i; ) { d += paramMatrix.get(b, b2) * paramMatrix.get(b1, b2) * arrayOfDouble[b2]; b2++; }
this.cvm.set(b1, b, d);
this.cvm.set(b, b1, d);
} 
} 
...
double[] arrayOfDouble = polynomialFunctionVector.function(arrayOfDouble1[b2]); double d; byte b;
for (d = 0.0D, b = 0; b < i; ) { d += arrayOfDouble3[b] * arrayOfDouble[b]; b++; }
