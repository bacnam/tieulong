...
this.colLabels = new Vector(this.colCount); byte b;
for (b = 0; b < this.rowCount; ) { this.rowLabels.add(categoricalTally1.getLabel(b)); b++; }
for (b = 0; b < this.colCount; ) { this.colLabels.add(categoricalTally2.getLabel(b)); b++; }
...
throw new IllegalArgumentException("Number of row labels do not match frequencies.");  byte b;
...
for (b = 0; b < this.rowCount; ) { this.rowLabels.add(paramArrayOfString1[b]); b++; }
for (b = 0; b < this.colCount; ) { this.colLabels.add(paramArrayOfString2[b]); b++; }
...
byte b;
for (b = 0; b < this.rowCount; ) { arrayOfString1[b] = this.rowLabels.get(b); b++; }
for (b = 0; b < this.colCount; ) { arrayOfString2[b] = this.colLabels.get(b); b++; }
...
arrayOfInt[b] = 0;
for (byte b1 = 0; b1 < this.colCount; ) { arrayOfInt[b] = arrayOfInt[b] + this.O[b][b1]; b1++; }
...
int[] arrayOfInt = new int[this.rowCount];
for (byte b = 0; b < this.rowCount; ) { arrayOfInt[b] = this.O[b][paramInt]; b++; }
return arrayOfInt;
}
...
arrayOfInt[b] = 0;
for (byte b1 = 0; b1 < this.rowCount; ) { arrayOfInt[b] = arrayOfInt[b] + this.O[b1][b]; b1++; }
...
stringBuffer.append("\nContingency table\n"); byte b1;
for (b1 = 0; b1 < this.colCount; ) { stringBuffer.append("\t" + getColumnLabel(b1)); b1++; }
for (byte b2 = 0; b2 < this.rowCount; b2++) {

stringBuffer.append("\n" + getRowLabel(b2));
for (b1 = 0; b1 < this.colCount; ) { stringBuffer.append("\t" + this.O[b2][b1]); b1++; }
