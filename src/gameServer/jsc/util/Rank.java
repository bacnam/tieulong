...
this.r = new int[paramInt]; int i;
for (i = 0; i < paramInt; ) { this.r[i] = i; i++; }

this.s = 0; this.t = 0;

double[] arrayOfDouble2 = new double[paramInt];
for (i = 0; i < paramInt; ) { arrayOfDouble2[i] = paramArrayOfdouble[i]; i++; }

Sort.sort(arrayOfDouble2, this.r, 0, paramInt - 1, true);

arrayOfDouble1[paramInt - 1] = (paramInt - 1); paramInt--;
...
double d = i + 0.5D * b1;
for (byte b2 = 0; b2 <= b1; ) { arrayOfDouble1[i + b2] = d; b2++; }
int k = b1 * (b1 + 1);
this.s += k;
this.t += k * (b1 + 2);
i += b1;
...
for (byte b = 0; b < paramInt; ) { this.rank[this.r[b]] = arrayOfDouble1[b] + 1.0D; b++; }
...
int[] arrayOfInt = rank.getSortIndexes(); byte b;
