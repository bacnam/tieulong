...
for (byte b = 0; b < paramInt; ) { this.randPerm[b] = b + 1; b++; }
reset();
}
...
for (byte b = 1; b <= this.n; ) { arrayOfInt[b - 1] = this.pi[b]; b++; }
...
this.pi[j] = this.pi[i];
this.pi[i] = k; int m;
for (m = i + 1; m <= this.n; ) { this.rho[m] = this.pi[m]; m++; }
for (m = i + 1; m <= this.n; ) { this.pi[m] = this.rho[this.n + i + 1 - m]; m++; }
this.count++;
return getPermutation();
}
...
public void reset() {
for (byte b = 1; b <= this.n; ) { this.pi[b] = b; b++; }
this.firstCall = true;
this.count = 1L;
this.hasNext = true;
}
