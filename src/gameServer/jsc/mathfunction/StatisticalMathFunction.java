...
return d;case 216: return (paramDouble1 != 0.0D && paramDouble2 != 0.0D) ? 1.0D : 0.0D;case 217: return (paramDouble1 != 0.0D || paramDouble2 != 0.0D) ? 1.0D : 0.0D;
...
throw new MathFunctionException("Attempted division by zero");  return Maths.fmod(paramDouble1, paramDouble2); }  throw new MathFunctionException("Implementation error"); } private void error_check(double paramDouble, int paramInt) throws MathFunctionException { if (Double.isNaN(paramDouble))
...
this.n = param2Int; this.x = new double[param2Int];
} public int getNumberOfVariables() { return this.n; }
public String getVariableName(int param2Int) { return "X" + (param2Int + 1); }
public double getVariableValue(int param2Int) { return this.x[param2Int]; } public void setVariableValue(int param2Int, double param2Double) {
...
if (str.charAt(0) == '?') { System.out.println(statisticalMathFunction.getSummary()); continue; }
...
try { x.setVariableValue(b, standardMathFunction.parse(str2)); }
catch (MathFunctionException mathFunctionException)
{ System.out.println(mathFunctionException.getMessage()); }
