...
while (l < this.uStar && d < paramDouble) { l++; d += this.prob[(int)(l - this.lStar)]; }
return l;
}
...
double d5 = 0.1D * paramDouble2; int j;
...
this.prob = new double[m]; int k;
for (j = this.lStar, k = 0; j <= this.uStar; ) { this.prob[k] = arrayOfDouble[j]; j++; k++; }

double d6 = 0.0D;

for (j = 0; j < m; ) { d6 += this.prob[j]; j++; }
if (d6 <= 0.0D) {
throw new IllegalArgumentException("Zero probabilities.");
}
for (j = 0; j < m; ) { this.prob[j] = this.prob[j] / d6; j++; }

this.mean = 0.0D; this.variance = 0.0D;
for (j = 0, k = this.lStar; k <= this.uStar; ) { this.mean += k * this.prob[j]; j++; k++; }
for (j = 0, k = this.lStar; k <= this.uStar; ) { this.variance += (k - this.mean) * (k - this.mean) * this.prob[j]; j++; k++; }
...
System.out.println("tolerance = " + d2); byte b;
...
long l1 = System.currentTimeMillis(); byte b1;
for (b1 = 0; b1 < i; ) { arrayOfInt1[b1] = (int)extendedHypergeometric.random(); b1++; }
long l2 = System.currentTimeMillis();
for (b1 = 0; b1 < i; ) { d5 += arrayOfInt1[b1]; b1++; }
double d3 = Maths.round(d5 / i, 4);
for (b1 = 0; b1 < i; ) { d6 += (arrayOfInt1[b1] - d3) * (arrayOfInt1[b1] - d3); b1++; }
...
for (b = 0; b < i; ) { arrayOfInt3[b] = (int)extendedHypergeometric.random(); b++; }
