...
/*     */     try {
/* 139 */       Context initialContext = new InitialContext();
...
/* 198 */     CyclicBuffer<E> cb = (CyclicBuffer<E>)this.cbTracker.getOrCreate(key, now);
...
/*     */           
/* 210 */           SenderRunnable senderRunnable = new SenderRunnable(cbClone, eventObject);
...
/*     */         } 
/*     */       } 
/* 217 */     } catch (EvaluationException ex) {
/* 218 */       this.errorCount++;
/* 219 */       if (this.errorCount < 4) {
/* 220 */         addError("SMTPAppender's EventEvaluator threw an Exception-", (Throwable)ex);
/*     */       }
/*     */     } 
/*     */ 
/*     */     
...
/* 281 */     } catch (AddressException e) {
/* 282 */       addError("Could not parse address [" + addressStr + "].", (Throwable)e);
/* 283 */       return null;
/*     */     } 
/*     */   }
/*     */   
...
/*     */     
/* 290 */     List<InternetAddress> iaList = new ArrayList<InternetAddress>();
/*     */     
/* 292 */     for (int i = 0; i < len; i++) {
/*     */       try {
/* 294 */         PatternLayoutBase<E> emailPL = this.toPatternLayoutList.get(i);
...
/* 301 */       } catch (AddressException e) {
/* 302 */         addError("Could not parse email address for [" + this.toPatternLayoutList.get(i) + "] for event [" + event + "]", (Throwable)e);
/* 303 */         return iaList;
/*     */       } 
/*     */     } 
/*     */     
...
/* 363 */         mimeMsg.setFrom((Address)getAddress(this.from));
/*     */       } else {
/* 365 */         mimeMsg.setFrom();
/*     */       } 
/*     */       
...
/*     */         return;
/*     */       } 
/* 376 */       InternetAddress[] toAddressArray = destinationAddresses.<InternetAddress>toArray(EMPTY_IA_ARRAY);
/* 377 */       mimeMsg.setRecipients(Message.RecipientType.TO, (Address[])toAddressArray);
/*     */       
...
/*     */       } 
/*     */       
/* 388 */       MimeMultipart mimeMultipart = new MimeMultipart();
/* 389 */       mimeMultipart.addBodyPart((BodyPart)part);
/* 390 */       mimeMsg.setContent((Multipart)mimeMultipart);
/*     */       
/* 392 */       mimeMsg.setSentDate(new Date());
/* 393 */       addInfo("About to send out SMTP message \"" + subjectStr + "\" to " + Arrays.toString((Object[])toAddressArray));
/* 394 */       Transport.send((Message)mimeMsg);
...
/*     */     }
/* 552 */     PatternLayoutBase<E> plb = makeNewToPatternLayout(to.trim());
...
/* 561 */     List<String> toList = new ArrayList<String>();
/* 562 */     for (PatternLayoutBase<E> plb : this.toPatternLayoutList) {
...
/* Location:              /Users/bacnam/Projects/TieuLongProject/gameserver/gameServer.jar!/ch/qos/logback/core/net/SMTPAppenderBase.class
