...
CallSite[] arrayOfCallSite = $getCallSiteArray(); String addMethod = null; if (!__$stMC && !BytecodeInterface8.disabledStandardMetaClass()) { GStringImpl gStringImpl1 = new GStringImpl(new Object[] { upperCaseFirstLetter(name) }, new String[] { "add", "" }); addMethod = ShortTypeHandling.castToString(gStringImpl1);
...
CallSite[] arrayOfCallSite = $getCallSiteArray(); NestingType nestingType1 = nestingType;
if (ScriptBytecodeAdapter.isCase(nestingType1, arrayOfCallSite[9].callGetProperty(NestingType.class))) {
Object object1 = arrayOfCallSite[10].call(Introspector.class, name); name = ShortTypeHandling.castToString(object1);
Object object2 = subComponent; ScriptBytecodeAdapter.setProperty(object2, null, component, ShortTypeHandling.castToString(new GStringImpl(new Object[] { name }, new String[] { "", "" })));
}
...
CallSite[] arrayOfCallSite = $getCallSiteArray(); if (BytecodeInterface8.isOrigInt() && BytecodeInterface8.isOrigZ() && !__$stMC && !BytecodeInterface8.disabledStandardMetaClass()) { if ((ScriptBytecodeAdapter.compareEqual(s, null) || ScriptBytecodeAdapter.compareEqual(arrayOfCallSite[14].call(s), Integer.valueOf(0))))
return s;  } else if ((ScriptBytecodeAdapter.compareEqual(s, null) || ScriptBytecodeAdapter.compareEqual(arrayOfCallSite[13].call(s), Integer.valueOf(0)))) { return s; }
