...
bo.setInstanceMaxLevelAll(0); byte b; int i; InstanceType[] arrayOfInstanceType;
for (i = (arrayOfInstanceType = InstanceType.values()).length, b = 0; b < i; ) { InstanceType type = arrayOfInstanceType[b];
...
try {
InstanceInfoBO bo = getOrCreate(); byte b; int i; InstanceType[] arrayOfInstanceType;
for (i = (arrayOfInstanceType = InstanceType.values()).length, b = 0; b < i; ) { InstanceType type = arrayOfInstanceType[b];
if (type == InstanceType.EquipInstance) {
bo.setChallengTimes(type.ordinal(), ((RefVIP)RefDataMgr.get(RefVIP.class, Integer.valueOf(this.player.getVipLevel()))).EquipInstanceTimes);
}
