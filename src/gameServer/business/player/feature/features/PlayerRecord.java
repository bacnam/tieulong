...
try {
PlayerRecordBO record = getOrCreate(); byte b; int i; ConstEnum.DailyRefresh[] arrayOfDailyRefresh;
for (i = (arrayOfDailyRefresh = ConstEnum.DailyRefresh.values()).length, b = 0; b < i; ) { ConstEnum.DailyRefresh dr = arrayOfDailyRefresh[b];
switch (dr) {
case PackageBuyTimes:
break;
default:
record.setValue(dr.ordinal(), 0); break;
} 
b++; }
