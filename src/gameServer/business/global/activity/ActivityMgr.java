...
private static Map<ActivityType, String> type2Class = new HashMap<>(); public static Comparator<Activity> Sorter; public long pid; public int icon; public String name; public int lv; public int vipLv; public long guildID; public String guildName; public int serverId; public int maxPower;
...
return (leftend != 0 && leftend > nowsec && rightend != 0 && rightend > nowsec) ? (leftend - rightend) : (

(left.bo.getCloseTime() < nowsec && right.bo.getCloseTime() < nowsec) ? (left.bo.getCloseTime() - right.bo.getCloseTime()) : (right.bo.getCloseTime() - left.bo.getCloseTime()));
...
return instance; }
public void activityStatusCheck() { for (Map.Entry<ActivityType, List<Activity>> list : this.activities.entrySet()) {
...
nextMin.longValue()); } public static <ACT extends Activity> ACT getLastActivity(Class<ACT> clazz) { ActivityType type = calss2Type.get(clazz.getName());
List<Activity> actList = (getInstance()).activities.get(type);
if (actList.size() > 1) {
