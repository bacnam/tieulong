...
int i = 0; while (true) { if (i < partitionSize) {
...
nextCheckInMs = tmp;  }  } else { long tmp = this.idleConnectionTestPeriodInMs - currentTimeInMs - connection.getConnectionLastResetInMs(); long tmp2 = this.idleMaxAgeInMs - currentTimeInMs - connection.getConnectionLastUsedInMs(); if (this.idleMaxAgeInMs > 0L) tmp = Math.min(tmp, tmp2);  if (tmp < nextCheckInMs) nextCheckInMs = tmp;
}
