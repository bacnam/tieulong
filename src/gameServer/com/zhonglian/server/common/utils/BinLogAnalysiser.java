...
File root = new File("./");
long count = 0L;
long filecount = 0L; byte b; int i; File[] arrayOfFile;
for (i = (arrayOfFile = root.listFiles()).length, b = 0; b < i; ) { File file = arrayOfFile[b];
...
Exception exception1 = null, exception2 = null; try { FileWriter output = new FileWriter(new File("analysie.out"));

try { for (Map.Entry<String, Map<Long, Long>> pair : analysie.entrySet()) {
...
finally { if (output != null) output.close();  }  } finally { exception2 = null; if (exception1 == null) { exception1 = exception2; } else if (exception1 != exception2) { exception1.addSuppressed(exception2); }
}
...
timestamp = Integer.parseInt(line.replace("SET TIMESTAMP=", "").replace(";", "")); return;
} 
