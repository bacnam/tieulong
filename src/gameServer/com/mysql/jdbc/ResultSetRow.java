...
/*  432 */           if (length > 7)
/*      */           {
/*  434 */             nanos = (bits[offset + 7] & 0xFF | (bits[offset + 8] & 0xFF) << 8 | (bits[offset + 9] & 0xFF) << 16 | (bits[offset + 10] & 0xFF) << 24) * 1000;
/*      */           }
/*      */         } 
/*      */         break;
/*      */ 
...
/*  574 */     long valueAsLong = (bits[offset + 0] & 0xFF) | (bits[offset + 1] & 0xFF) << 8L | (bits[offset + 2] & 0xFF) << 16L | (bits[offset + 3] & 0xFF) << 24L | (bits[offset + 4] & 0xFF) << 32L | (bits[offset + 5] & 0xFF) << 40L | (bits[offset + 6] & 0xFF) << 48L | (bits[offset + 7] & 0xFF) << 56L;
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*  612 */     long valueAsLong = (bits[offset + 0] & 0xFF) | (bits[offset + 1] & 0xFF) << 8L | (bits[offset + 2] & 0xFF) << 16L | (bits[offset + 3] & 0xFF) << 24L | (bits[offset + 4] & 0xFF) << 32L | (bits[offset + 5] & 0xFF) << 40L | (bits[offset + 6] & 0xFF) << 48L | (bits[offset + 7] & 0xFF) << 56L;
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*      */ 
/*      */         
/*  980 */         SQLWarning precisionLost = new SQLWarning(Messages.getString("ResultSet.Precision_lost_converting_TIMESTAMP_to_Time_with_getTime()_on_column__261") + columnIndex + "(" + timeColField + ").");
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*      */       
/*      */       }
/*  989 */       else if (timeColField.getMysqlType() == 12) {
...
/*      */ 
/*      */         
/*  995 */         SQLWarning precisionLost = new SQLWarning(Messages.getString("ResultSet.Precision_lost_converting_DATETIME_to_Time_with_getTime()_on_column__264") + (columnIndex + 1) + "(" + timeColField + ").");
/*      */ 
/*      */ 
/*      */       
/*      */       }
...
/*      */     
/*      */     }
/* 1044 */     catch (RuntimeException ex) {
/* 1045 */       SQLException sqlEx = SQLError.createSQLException(ex.toString(), "S1009", this.exceptionInterceptor);
/*      */       
...
/* 1068 */         boolean hasDash, hasColon; int j; boolean allZeroTimestamp = true;
/*      */         
...
/*      */               
/* 1174 */               for (int k = 0; k < length; k++) {
/* 1175 */                 if (timestampAsBytes[offset + k] == 46) {
/* 1176 */                   decimalIndex = k;
/*      */                 }
/*      */               } 
/*      */               
...
/* 1248 */             hasDash = false;
/*      */             
/* 1250 */             for (j = 0; j < length; j++) {
/* 1251 */               if (timestampAsBytes[offset + j] == 45) {
/* 1252 */                 hasDash = true;
/*      */                 
/*      */                 break;
/*      */               } 
...
/* 1294 */               if (timestampAsBytes[offset + j] == 58) {
/* 1295 */                 hasColon = true;
/*      */                 
/*      */                 break;
/*      */               } 
...
/* 1404 */       SQLException sqlEx = SQLError.createSQLException("Cannot convert value '" + getString(columnIndex, "ISO8859_1", conn) + "' from column " + (columnIndex + 1) + " to TIMESTAMP.", "S1009", this.exceptionInterceptor);
/*      */ 
/*      */ 
/*      */       
...
/* Location:              /Users/bacnam/Projects/TieuLongProject/gameserver/gameServer.jar!/com/mysql/jdbc/ResultSetRow.class
