...
boolean hasConstructor = false; int k;
...
generateMethod(this.className, this.fqClassName, this.methods[k].getName(), returnType, this.methods[k].getParamTypeDescriptors(), modifiers, cw);

boolean isStatic = ((modifiers & 0x8) > 0);
boolean overridden = classContainsMethod(this.superClass, this.methods[k].getName(), this.methods[k].getParamTypeDescriptors());

if (!isStatic && overridden) {
generateSuperDelegateMethod(this.superClassName, this.methods[k].getName(), returnType, this.methods[k].getParamTypeDescriptors(), modifiers, cw);
}
} 
} 
...
cv.visitMethodInsn(182, "bsh/This", "invokeMethod", Type.getMethodDescriptor(Type.getType(Object.class), new Type[] { Type.getType(String.class), Type.getType(Object[].class), Type.getType(Interpreter.class), Type.getType(CallStack.class), Type.getType(SimpleNode.class), Type.getType(boolean.class) }));
...
doSwitchBranch(index, this.superClassName, getTypeDescriptors(this.superConstructors[j].getParameterTypes()), endLabel, labels, consArgsVar, cv);
}

for (j = 0; j < this.constructors.length; j++, index++) {
doSwitchBranch(index, this.fqClassName, this.constructors[j].getParamTypeDescriptors(), endLabel, labels, consArgsVar, cv);
}
...
Class[][] candidates = new Class[constructors.length][]; int j;
...
public float getFloat() { return ((Float)next()).floatValue(); } public Object getObject() {
return next();
}
}
