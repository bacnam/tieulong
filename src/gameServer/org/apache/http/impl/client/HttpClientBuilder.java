...
return s.split(" *, *"); } public CloseableHttpClient build() { PoolingHttpClientConnectionManager poolingHttpClientConnectionManager; DefaultConnectionReuseStrategy defaultConnectionReuseStrategy; RetryExec retryExec;
...
SSLConnectionSocketFactory sSLConnectionSocketFactory; LayeredConnectionSocketFactory sslSocketFactoryCopy = this.sslSocketFactory;
if (sslSocketFactoryCopy == null) {
DefaultHostnameVerifier defaultHostnameVerifier; String[] supportedProtocols = this.systemProperties ? split(System.getProperty("https.protocols")) : null;
...
DefaultSchemePortResolver defaultSchemePortResolver; SchemePortResolver schemePortResolverCopy = this.schemePortResolver;
...
redirectExec = new RedirectExec((ClientExecChain)retryExec, (HttpRoutePlanner)defaultRoutePlanner, redirectStrategyCopy);
} 

ServiceUnavailableRetryStrategy serviceUnavailStrategyCopy = this.serviceUnavailStrategy;
if (serviceUnavailStrategyCopy != null) {
serviceUnavailableRetryExec = new ServiceUnavailableRetryExec((ClientExecChain)redirectExec, serviceUnavailStrategyCopy);
}

if (this.backoffManager != null && this.connectionBackoffStrategy != null) {
backoffStrategyExec = new BackoffStrategyExec((ClientExecChain)serviceUnavailableRetryExec, this.connectionBackoffStrategy, this.backoffManager);
}
...
RFC6265CookieSpecProvider rFC6265CookieSpecProvider1 = new RFC6265CookieSpecProvider(RFC6265CookieSpecProvider.CompatibilityLevel.RELAXED, publicSuffixMatcherCopy);

RFC6265CookieSpecProvider rFC6265CookieSpecProvider2 = new RFC6265CookieSpecProvider(RFC6265CookieSpecProvider.CompatibilityLevel.STRICT, publicSuffixMatcherCopy);

registry2 = RegistryBuilder.create().register("default", defaultCookieSpecProvider).register("best-match", defaultCookieSpecProvider).register("compatibility", defaultCookieSpecProvider).register("standard", rFC6265CookieSpecProvider1).register("standard-strict", rFC6265CookieSpecProvider2).register("netscape", new NetscapeDraftSpecProvider()).register("ignoreCookies", new IgnoreSpecProvider()).build();
} 
...
final IdleConnectionEvictor connectionEvictor = new IdleConnectionEvictor((HttpClientConnectionManager)poolingHttpClientConnectionManager1, (this.maxIdleTime > 0L) ? this.maxIdleTime : 10L, (this.maxIdleTimeUnit != null) ? this.maxIdleTimeUnit : TimeUnit.SECONDS);
