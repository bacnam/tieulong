...
for (Iterator<Long> iterator = faighters.iterator(); iterator.hasNext(); ) { long i = ((Long)iterator.next()).longValue();
Player tmp_player = PlayerMgr.getInstance().getPlayer(i);
if (((DroiyanFeature)player.getFeature(DroiyanFeature.class)).isEnemy(i)) {
players.add(new StealGoldEnemy(((PlayerBase)tmp_player.getFeature(PlayerBase.class)).summary(), true, null)); continue;
} 
...
for (Iterator<Integer> iterator1 = feature.getMoneyList().iterator(); iterator1.hasNext(); ) { int ext = ((Integer)iterator1.next()).intValue();
