...
public class SyncTimerTask
    ...
    public void run() {
        ...
        for (i = (arrayOfStackTraceElement = this.task.getException().getStackTrace()).length, b = 0; b < i; ) {
            StackTraceElement st = arrayOfStackTraceElement[b];
            dept++;
            if (dept > ignoreDept) {
                info.append("  ");
        ...
        if (runningThread != null) {
            ...
            if (stackTrace == null) {
                ...
                for (int j = (arrayOfStackTraceElement1 = stackTrace).length; i < j; ) {
