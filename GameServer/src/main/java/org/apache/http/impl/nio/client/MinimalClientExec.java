...
this.httpProcessor.process((HttpRequest)request, (HttpContext)localContext);
}
...
this.connmgr.startRoute(localConn, route, (HttpContext)localContext);
this.connmgr.routeComplete(localConn, route, (HttpContext)localContext);
} 
...
return (HttpRequest)state.getCurrentRequest();
}
...
this.httpProcessor.process(response, (HttpContext)localContext);
...
if (this.connReuseStrategy.keepAlive(response, (HttpContext)localContext)) {
long validDuration = this.keepaliveStrategy.getKeepAliveDuration(response, (HttpContext)localContext);
