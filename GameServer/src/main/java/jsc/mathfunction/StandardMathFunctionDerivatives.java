...
for (byte b = 0; b <= paramInt2; ) { arrayOfDouble2[b] = toDerivative(b, arrayOfDouble1[b]); b++; }
...
else { if (this.T[j][paramInt2] == 0.0D) {
...
int k; if (paramDouble2 == (k = (int)paramDouble2)) {
...
return 2;
} public String getVariableName(int param2Int) { return (param2Int == 0) ? "X" : "Y"; } public double getVariableValue(int param2Int) {
...
if (str1.charAt(0) == '?') { System.out.println(standardMathFunctionDerivatives.getSummary());
continue; }
...
System.out.println("Expression must be a function of X to evaluate Taylor coeffs."); continue;
} 
...
try { i = Integer.parseInt(str1); break; }
catch (NumberFormatException numberFormatException) { System.out.println("Invalid input: " + numberFormatException.getMessage()); }
...
System.out.println("P(x) = " + arrayOfDouble[0]); byte b;
for (b = 1; b <= i; b++) {
...
for (b = 1; b <= i; ) { d4 += arrayOfDouble[b] * Math.pow(d3, b); b++; }
...
try { str = param1BufferedReader.readLine(); }
catch (IOException iOException) { System.out.println(iOException.getMessage()); continue; }
try { d = standardMathFunction.parse(str); break; }
catch (MathFunctionException mathFunctionException) { System.out.println(mathFunctionException.getMessage()); } 
}  return d;
}
}
}
