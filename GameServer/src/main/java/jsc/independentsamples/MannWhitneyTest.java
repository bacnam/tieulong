...
this.RA = 0.0D; int i;
for (i = 0; i < this.nA; i++) {
double d = this.ranks.getRank(i); this.RA += d; arrayOfDouble1[i] = d;
} 

this.RB = 0.0D;
for (byte b = 0; i < this.N; b++, i++) {
double d = this.ranks.getRank(i); this.RB += d; arrayOfDouble2[b] = d;
} 
...
if (arrayOfInt[b2] == 1) {
this.permutedRanksA[b1] = this.originalRanks[b2]; b1++;
} 
} 
...
double d1, d2 = 0.0D;
for (byte b = 0; b < this.nA; ) { d2 += paramArrayOfdouble[b]; b++; }
