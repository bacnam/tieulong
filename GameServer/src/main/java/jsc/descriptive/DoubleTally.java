...
for (b1 = 0; b1 < b2; ) { this.freq.add(new Integer(0)); b1++; }
...
if (Math.abs(paramDouble - d1) <= this.tolerance) { addExisting(0); return 0; }
if (paramDouble < d1) { addNew(0, paramDouble); return -1; }
if (Math.abs(paramDouble - d2) <= this.tolerance) { addExisting(i - 1); return i - 1; }
if (paramDouble > d2) { addEnd(paramDouble); return -1; }
...
if (Math.abs(paramDouble - d) <= this.tolerance) { addExisting(m); return m; }
if (paramDouble < d) {
k = m; continue;
} 
j = m;
...
if (Math.abs(paramDouble - d3) <= this.tolerance) { addExisting(k); return k; }
...
DoubleTally doubleTally = new DoubleTally(); byte b;
for (b = 0; b < arrayOfDouble.length; ) { doubleTally.addValue(arrayOfDouble[b]); b++; }
