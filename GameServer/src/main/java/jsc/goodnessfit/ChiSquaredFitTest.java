...
int j = 0;
for (byte b = 0; b < i; ) { j += paramArrayOfint[b]; b++; }
...
if (this.E[this.k - 1] >= 5.0D || this.k <= 2) continue;  poolBins(this.k - 2, this.k - 1);
}
...
int i;
for (i = 0; i < paramInt1; ) { arrayOfDouble[i] = this.probs[i]; arrayOfInt[i] = this.O[i]; i++; }
for (i = paramInt1; i <= paramInt2; ) { d += this.probs[i]; m += this.O[i]; i++; }
arrayOfDouble[paramInt1] = d; arrayOfInt[paramInt1] = m;
for (i = paramInt1 + 1; i < k; ) { arrayOfDouble[i] = this.probs[i + j]; arrayOfInt[i] = this.O[i + j]; i++; }
