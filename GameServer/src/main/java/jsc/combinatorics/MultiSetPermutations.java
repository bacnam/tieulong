...
for (byte b = 1; b <= this.N; ) { arrayOfInt[b - 1] = this.A[this.k][b]; b++; }
...
int k = b + 1;
if (this.A[i][b] != i || this.A[i][k] != i + 1) { b++; continue; }
int m = b - 2;
...
int j = this.r[b]; int m;
for (m = 1; m <= j; ) { this.A[b][m] = b; m++; }
int k = this.r[b] + 1;
for (m = k; m <= i; ) { this.A[b][m] = b + 1; m++; }
i -= this.r[b];
} 
...
private void step5() {
byte b;
for (b = 1; b <= this.N; ) { this.A[this.k][b] = this.A[1][b]; b++; }
