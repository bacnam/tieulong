...
try { return this.decimalFormat.parse(paramString); }
catch (ParseException parseException) { return this.scientificFormat.parse(paramString); }
...
for (byte b = 0; b < paramInt - 6; ) { stringBuffer.append('#'); b++; }
...
try { number = maxWidthFormat.parse(str); }
catch (ParseException parseException) { System.out.println("Cannot parse " + str); }
