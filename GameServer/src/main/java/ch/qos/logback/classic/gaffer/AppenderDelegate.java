...
public class AppenderDelegate
    ...
    public AppenderDelegate(Appender appender) {
        super(appender);
        Map<String, Appender<?>> map = ScriptBytecodeAdapter.createMap(new Object[0]);
    }

    public AppenderDelegate(Appender appender, List appenders) {
        super(appender);
        Map<String, Appender<?>> map = ScriptBytecodeAdapter.createMap(new Object[0]);
        Object object = arrayOfCallSite[0].call(appenders, new _closure1(this, this));
        this.appendersByName = (Map<String, Appender<?>>) ScriptBytecodeAdapter.castToType(object, Map.class);
    }
    ...
    public void appenderRef(String name) {
        CallSite[] arrayOfCallSite = $getCallSiteArray();
        if (!DefaultTypeTransformation.booleanUnbox(arrayOfCallSite[1].call(AppenderAttachable.class, arrayOfCallSite[2].callGetProperty(arrayOfCallSite[3].callGroovyObjectGetProperty(this))))) {
