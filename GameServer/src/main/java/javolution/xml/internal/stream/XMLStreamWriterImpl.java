...
public final class XMLStreamWriterImpl
    ...
    XMLStreamWriterImpl(XMLOutputFactoryImpl factory) {
        this._noChar = new CharArray("");

        this._tmpCharArray = new CharArray();

        this._autoPrefix = new TextBuilder();
    ...
    public void writeProcessingInstruction(CharSequence target, CharSequence data) throws XMLStreamException {
        if (target == null) throw new XMLStreamException("Target cannot be null");
        if (data == null) throw new XMLStreamException("Data cannot be null");
        if (this._isElementOpen) closeOpenTag();
        writeNoEscape("<?");
    ...
    public void writeDTD(CharSequence dtd) throws XMLStreamException {
        if (dtd == null) throw new XMLStreamException("DTD cannot be null");
        if (this._nesting > 0)
    ...
    public void writeStartDocument(CharSequence encoding, CharSequence version) throws XMLStreamException {
        if (this._nesting > 0) throw new XMLStreamException("Not in document root");
    ...
    public void writeCharacters(char[] text, int start, int length) throws XMLStreamException {
        ...
        writeCharacters((CharSequence) this._tmpCharArray);
    }

    private void resizeElemStack() {
        int oldLength = this._qNames.length;
    ...
    public CharSequence getPrefix(CharSequence uri) throws XMLStreamException {
        return (CharSequence) this._namespaces.getPrefix(uri);
    }

    public void setPrefix(CharSequence prefix, CharSequence uri) throws XMLStreamException {
        this._namespaces.setPrefix(prefix, (uri == null) ? (CharSequence) this._namespaces._nullNsURI : uri, false);
    }

    public void setDefaultNamespace(CharSequence uri) throws XMLStreamException {
        setPrefix((CharSequence) this._namespaces._defaultNsPrefix, uri);
    }

    public Object getProperty(String name) throws IllegalArgumentException {
        if (name.equals("javolution.xml.stream.isRepairingNamespaces")) return new Boolean(this._isRepairingNamespaces);
        if (name.equals("javolution.xml.stream.repairingPrefix")) return this._repairingPrefix;
    ...
    private void writeNewElement(CharSequence prefix, CharSequence localName, CharSequence namespaceURI) throws XMLStreamException {
        ...
        if (this._indentation != null) {
            ...
            for (int i = 0; i < this._indentationLevel; i++) writeNoEscape(this._indentation);
        }
        write('<');
        this._isElementOpen = true;
        if (++this._nesting >= this._qNames.length) resizeElemStack();
        this._namespaces.push();
        TextBuilder qName = this._qNames[this._nesting].clear();
        if (namespaceURI != null && !this._namespaces._defaultNamespace.equals(namespaceURI)) {
    ...
    private void writeAttributeOrNamespace(CharSequence prefix, CharSequence namespaceURI, CharSequence localName, CharSequence value) throws XMLStreamException {
        if (!this._isElementOpen) throw new IllegalStateException("No open start element");
        write(' ');
        if (namespaceURI != null && !this._namespaces._defaultNamespace.equals(namespaceURI)) {
            if (this._isRepairingNamespaces) {
                prefix = getRepairedPrefix(prefix, namespaceURI);
    ...
    private void writeNamespaces() throws XMLStreamException {
        ...
        for (int i = i0; i < i2; i++) {
            if ((this._isRepairingNamespaces && i < i1 && !this._namespaces._prefixesWritten[i]) || (i >= i1 && this._namespaces._prefixesWritten[i])) {
                if (this._isRepairingNamespaces) {
                    CharArray prefix = this._namespaces.getPrefix((CharSequence) this._namespaces._namespaces[i], i);
                    if (this._namespaces._prefixes[i].equals(prefix)) continue;
                }
                if (this._namespaces._prefixes[i].length() == 0) {
                    writeAttributeOrNamespace(null, null, (CharSequence) this._namespaces._xmlns, (CharSequence) this._namespaces._namespaces[i]);
                } else {
                    writeAttributeOrNamespace((CharSequence) this._namespaces._xmlns, (CharSequence) this._namespaces._xmlnsURI, (CharSequence) this._namespaces._prefixes[i], (CharSequence) this._namespaces._namespaces[i]);
                }
            }
            continue;
        }
    ...
    private CharSequence getRepairedPrefix(CharSequence prefix, CharSequence namespaceURI) throws XMLStreamException {
        ...
        if (prefixForURI != null && (prefix == null || prefixForURI.equals(prefix))) return (CharSequence) prefixForURI;
        if (prefix == null || prefix.length() == 0)
            textBuilder = this._autoPrefix.clear().append(this._repairingPrefix).append(this._autoNSCount++);
        this._namespaces.setPrefix((CharSequence) textBuilder, namespaceURI, true);
        return (CharSequence) textBuilder;
    }
