...
public class Guild {
    ...
    protected final BaseMutexManager m_mutex = new BaseMutexManager();
    ...
    private Map<GuildRankType, GuildRank> ranks;
    ...
    public void broadcastMember(long... cids) {
        ...
        List<core.network.proto.Guild.member> members = Lists.newArrayList();
        byte b;
        int i;
        long[] arrayOfLong;
        for (i = (arrayOfLong = cids).length, b = 0; b < i; ) {
            long cid = arrayOfLong[b];
            if (this.members.contains(Long.valueOf(cid))) {

                members.add(((GuildMemberFeature) playerMgr.getPlayer(cid).getFeature(GuildMemberFeature.class)).memberInfo());
            }
            b++;
        }
    ...
    public int updatePower() {
        ...
        for (Iterator<Long> iterator = this.members.iterator(); iterator.hasNext(); ) {
            long pid = ((Long) iterator.next()).longValue();
    ...
    public void init() {
        ...
        for (Class<?> clz : clazzs) {
            ...
            try {
                ...
                byte b;
                int i;
                GuildRankType[] arrayOfGuildRankType;
                for (i = (arrayOfGuildRankType = types.value()).length, b = 0; b < i; ) {
                    GuildRankType type = arrayOfGuildRankType[b];
    ...
    public void clearPlayerData(Player player) {
        ...
        GuildRanks types = GuildNormalRank.class.<GuildRanks>getAnnotation(GuildRanks.class);
        byte b;
        int i;
        GuildRankType[] arrayOfGuildRankType;
        for (i = (arrayOfGuildRankType = types.value()).length, b = 0; b < i; ) {
