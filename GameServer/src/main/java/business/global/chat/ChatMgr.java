...
this.allWorldChat = new LinkedList<>(); loadChatMessages(); broadCastWorldChat(); this.chatRecordCount = RefDataMgr.getFactor("ChatRecordCount", 30);
...
); } public boolean loadChatMessages() { List<ChatMessageBO> all = BM.getBM(ChatMessageBO.class).findAll();
Collections.sort(all, (o1, o2) -> o1.getSendTime() - o2.getSendTime());

for (ChatMessageBO bo : all) {
Player player; long guildId; Queue<ChatMessageBO> guildchat; Queue<ChatMessageBO> sender; Queue<ChatMessageBO> company; Queue<ChatMessageBO> senderGM; Queue<ChatMessageBO> companyGM; switch (ChatType.values()[bo.getChatType()]) {
case CHATTYPE_WORLD:
