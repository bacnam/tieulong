...
update0(type, (bo, ref, values) -> { int addCount = values[0].intValue(); synchronized (bo) { long count = bo.getValue(); bo.saveValue(count + addCount); taskActive(bo, ref); }  }new Integer[] {
...
update0(type, (bo, ref, values) -> { int newvalue = values[0].intValue(); synchronized (bo) { long count = bo.getValue(); bo.saveValue(Math.max(count, newvalue)); taskActive(bo, ref); }  }new Integer[] {
...
update0(type, (bo, ref, values) -> { int newvalue = values[0].intValue(); synchronized (bo) { long count = bo.getValue(); if (count == 0L) count = newvalue;  bo.saveValue(Math.min(count, newvalue)); taskActive(bo, ref); }  }new Integer[] {
