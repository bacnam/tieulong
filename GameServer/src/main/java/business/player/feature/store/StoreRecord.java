...
public class StoreRecord
    ...
    public StoreRecordBO getOrCreate() {
        ...
        synchronized (this) {
            ...
            record.setPid(this.player.getPid());
            byte b;
            int i;
            StoreType[] arrayOfStoreType;
            for (i = (arrayOfStoreType = StoreType.values()).length, b = 0; b < i; ) {
                StoreType storeType = arrayOfStoreType[b];
                record.setFreeRefreshTimes(storeType.ordinal(), ((Integer) RefStoreRefresh.FreeStoreTimes.get(storeType)).intValue());
                b++;
            }
