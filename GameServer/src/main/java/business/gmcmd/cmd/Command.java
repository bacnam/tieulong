...
public class Command {
    ...
    private Object[] parseParams(Player player, Object[] args) throws Exception {
        ...
        int parameCount = this.cmd.getParameterCount();
        byte b;
        int j;
        Class[] arrayOfClass1;
        for (j = (arrayOfClass1 = type).length, b = 0; b < j; ) {
    ...
    private Object parseParam(Class<?> clazz, Object object) throws Exception {
        ...
        if (List.class.isAssignableFrom(clazz)) {
            ...
            for (i = (arrayOfString1 = subparams).length, b = 0; b < i; ) {
                String p = arrayOfString1[b];
                list.add(parseParam(ptClass, p));
                b++;
            }
    ...
    private String getParamString() {
        ...
        for (i = (arrayOfParameter = this.cmd.getParameters()).length, b = 0; b < i; ) {
            Parameter parameter = arrayOfParameter[b];
            if (parameter.getType() != Player.class) {

                stringBuilder.append(parameter.getType().getSimpleName().toLowerCase()).append(" ");
            }
            b++;
        }
