...
public class StatementImpl
    ...
    private boolean execute(String sql, boolean returnGeneratedKeys) throws SQLException {
        ...
        synchronized (locallyScopedConn) {
            ...
            if (returnGeneratedKeys) this.lastQueryIsOnDupKeyUpdate = containsOnDuplicateKeyInString(sql);
            resetCancelledState();
            ...
            try {
                ...
                if (useServerFetch()) {
                    ...
                    try {
                        ...
                        if (locallyScopedConn.getCacheResultSetMetadata()) {
                            ...
                            if (cachedMetaData != null) cachedFields = cachedMetaData.fields;
                        }
                ...
                if (rs != null) {
                    ...
                    if (rs.reallyResult()) if (cachedMetaData != null) {
    ...
    protected void resetCancelledState() throws SQLException {
        synchronized (checkClosed()) {
            if (this.cancelTimeoutMutex == null) return;
            synchronized (this.cancelTimeoutMutex) {
                            ...
                            try {
                                ...
                                if (this.continueBatchOnError && !(ex instanceof MySQLTimeoutException) && !(ex instanceof MySQLStatementCancelledException) && !hasDeadlockOrTimeoutRolledBackTx(ex)) {
                                    ...
                                    if (hasDeadlockOrTimeoutRolledBackTx(ex)) {
                                        for (int j = 0; j < newUpdateCounts.length; j++) newUpdateCounts[j] = -3;
                                    } else {
    ...
    private int[] executeBatchUsingMultiQueries(boolean multiQueriesEnabled, int nbrCommands, int individualStatementTimeout) throws SQLException {
        MySQLConnection locallyScopedConn = checkClosed();
        synchronized (locallyScopedConn) {
            if (!multiQueriesEnabled) locallyScopedConn.getIO().enableMultiQueries();
            ...
            try {
                ...
                if (this.doEscapeProcessing) escapeAdjust = 2;
                SQLException sqlEx = null;
    ...
    protected SQLException handleExceptionForBatch(int endOfBatchIndex, int numValuesPerBatch, int[] updateCounts, SQLException ex) throws BatchUpdateException {
        SQLException sqlEx;
        for (int j = endOfBatchIndex; j > endOfBatchIndex - numValuesPerBatch; j--) updateCounts[j] = -3;
        if (this.continueBatchOnError && !(ex instanceof MySQLTimeoutException) && !(ex instanceof MySQLStatementCancelledException) && !hasDeadlockOrTimeoutRolledBackTx(ex)) {
            sqlEx = ex;
        } else {
    ...
    protected Calendar getCalendarInstanceForSessionOrNew() throws SQLException {
        synchronized (checkClosed()) {
            if (this.connection != null) return this.connection.getCalendarInstanceForSessionOrNew();
    ...
    public ResultSet getGeneratedKeys() throws SQLException {
        synchronized (checkClosed()) {
            ...
            if (this.batchedGeneratedKeys == null) {
                if (this.lastQueryIsOnDupKeyUpdate) return getGeneratedKeysInternal(1);
    ...
    public long getLongUpdateCount() {
        try {
            synchronized (checkClosed()) {
                if (this.results == null) return -1L;
                if (this.results.reallyResult()) return -1L;
    ...
    public boolean getMoreResults(int current) throws SQLException {
        synchronized (checkClosed()) {
            ...
            if (streamingMode && this.results.reallyResult()) while (this.results.next()) ;
            ResultSetInternalMethods nextResultSet = this.results.getNextResultSet();
            switch (current) {
                case 1:
                    if (this.results != null) {
                        if (!streamingMode) this.results.close();
                ...
                case 3:
                    ...
                    break;
                case 2:
                    if (!this.connection.getDontTrackOpenResources()) this.openResults.add(this.results);
                    this.results.clearNextResult();
                    break;
                default:
    ...
    public int getUpdateCount() throws SQLException {
        synchronized (checkClosed()) {
            if (this.results == null) return -1;
            if (this.results.reallyResult()) return -1;
            int truncatedUpdateCount = 0;
    ...
    public SQLWarning getWarnings() throws SQLException {
        synchronized (checkClosed()) {
            if (this.clearWarningsCalled) return null;
    ...
    protected void realClose(boolean calledExplicitly, boolean closeOpenResults) throws SQLException {
        ...
        synchronized (locallyScopedConn) {
            ...
            if (closeOpenResults) closeOpenResults = !this.holdResultsOpenOverClose;
            if (closeOpenResults) {
    ...
    public boolean isClosed() throws SQLException {
        try {
            ...
            if ("08003".equals(sqlEx.getSQLState())) return true;
            throw sqlEx;
        }
    }
