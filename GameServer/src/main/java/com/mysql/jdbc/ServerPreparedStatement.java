...
public class ServerPreparedStatement
    ...
    protected ServerPreparedStatement(MySQLConnection conn, String sql, String catalog, int resultSetType, int resultSetConcurrency) throws SQLException {
        ...
        if (lim_id != -1) {
            ...
            if (lim_id > 0 && (sql.charAt(lim_id - 1) == '`' || StringUtils.isValidIdChar(sql.charAt(lim_id - 1))))
                hasPreviosIdChar = true;
            if (lim_id + 5 < sql.length() && (sql.charAt(lim_id + 5) == '`' || StringUtils.isValidIdChar(sql.charAt(lim_id + 5))))
                hasFollowingIdChar = true;
            if (!hasPreviosIdChar && !hasFollowingIdChar) this.hasLimitClause = true;
        } else {
            this.hasLimitClause = false;
        }
        String statementComment = this.connection.getStatementComment();
        this.originalSql = (statementComment == null) ? sql : ("" + sql);
        ...
        synchronized (checkClosed()) {
            ...
            try {
                ...
                for (int i = 0; i < numParameters && i < ourNumParameters; i++) {
                    if (this.parameterBindings[i] != null) if ((this.parameterBindings[i]).isNull) {
    ...
    protected MySQLConnection checkClosed() throws SQLException {
        if (this.invalid) throw this.invalidationException;
    ...
    private void clearParametersInternal(boolean clearServerParameters) throws SQLException {
        ...
        if (this.parameterBindings != null) for (int i = 0; i < this.parameterCount; i++) {
            if (this.parameterBindings[i] != null && (this.parameterBindings[i]).isLongData) hadLongData = true;
    ...
    public void close() throws SQLException {
        try {
            ...
            if ("08003".equals(sqlEx.getSQLState())) return;
            throw sqlEx;
        }
    }
    ...
    protected ResultSetInternalMethods executeInternal(int maxRowsToRetrieve, Buffer sendPacket, boolean createStreamingResultSet, boolean queryIsSelectOnly, Field[] metadataFromCache, boolean isBatch) throws SQLException {
        synchronized (checkClosed()) {
            ...
            try {
                ...
                if (this.connection.getEnablePacketDebug()) this.connection.getIO().dumpPacketRingBuffer();
    ...
    byte[] getBytes(int parameterIndex) throws SQLException {
        synchronized (checkClosed()) {
            ...
            if (bindValue.isNull) return null;
            if (bindValue.isLongData) throw SQLError.notImplemented();
            if (this.outByteBuffer == null) this.outByteBuffer = new Buffer(this.connection.getNetBufferLength());
    ...
    public ResultSetMetaData getMetaData() throws SQLException {
        synchronized (checkClosed()) {
            if (this.resultFields == null) return null;
    ...
    protected void realClose(boolean calledExplicitly, boolean closeOpenResults) throws SQLException {
        ...
        synchronized (locallyScopedConn) {
            if (this.connection != null) {
                if (this.connection.getAutoGenerateTestcaseScript()) dumpCloseForTestcase();
                SQLException exceptionDuringClose = null;
                if (calledExplicitly && !this.connection.isClosed()) synchronized (this.connection) {
                    try {
                ...
                if (exceptionDuringClose != null) throw exceptionDuringClose;
            }
        }
    }
    ...
    protected void rePrepare() throws SQLException {
        synchronized (checkClosed()) {
            ...
            if (this.invalidationException != null) {
                ...
                this.parameterFields = null;
                this.resultFields = null;
                if (this.results != null) try {
                    this.results.close();
                } catch (Exception ex) {
                }
                if (this.generatedKeysResults != null) try {
                ...
                if (this.connection != null) {
                    if (this.maxRowsChanged) this.connection.unsetMaxRows(this);
                    if (!this.connection.getDontTrackOpenResources()) this.connection.unregisterStatement(this);
                }
            }
        }
    }
    ...
    private ResultSetInternalMethods serverExecute(int maxRowsToRetrieve, boolean createStreamingResultSet, Field[] metadataFromCache) throws SQLException {
        synchronized (checkClosed()) {
            ...
            if (mysql.shouldIntercept()) {
                ...
                if (interceptedResults != null) return interceptedResults;
            }
            if (this.detectedLongParameterSwitch) {
                boolean firstFound = false;
                long boundTimeToCheck = 0L;
                for (int m = 0; m < this.parameterCount - 1; m++) {
                    if ((this.parameterBindings[m]).isLongData) {
                        if (firstFound && boundTimeToCheck != (this.parameterBindings[m]).boundBeforeExecutionNum)
                        ...
                        firstFound = true;
                        boundTimeToCheck = (this.parameterBindings[m]).boundBeforeExecutionNum;
                    }
                }
                serverResetStatement();
            }
            ...
            for (i = 0; i < this.parameterCount; i++) {
                if ((this.parameterBindings[i]).isLongData) serverLongData(i, this.parameterBindings[i]);
            }
            if (this.connection.getAutoGenerateTestcaseScript()) dumpExecuteForTestcase();
            Buffer packet = mysql.getSharedSendPacket();
            packet.clear();
            ...
            for (int j = 0; j < nullCount; j++) packet.writeByte((byte) 0);
            byte[] nullBitsBuffer = new byte[nullCount];
            packet.writeByte(this.sendTypesToServer ? 1 : 0);
            if (this.sendTypesToServer)
                for (int m = 0; m < this.parameterCount; m++) packet.writeInt((this.parameterBindings[m]).bufferType);
            for (int k = 0; k < this.parameterCount; k++) {
                if (!(this.parameterBindings[k]).isLongData) if (!(this.parameterBindings[k]).isNull) {
                    storeBinding(packet, this.parameterBindings[k], mysql);
                } else {
            ...
            try {
                ...
                if (logSlowQueries || gatherPerformanceMetrics) {
                    ...
                    if (logSlowQueries) if (this.useAutoSlowLog) {
                    ...
                    if (queryWasSlow) {
                        ...
                        this.eventSink.consumeEvent(new ProfilerEvent((byte) 6, "", this.currentCatalog, this.connection.getId(), getId(), 0, System.currentTimeMillis(), elapsedTime, mysql.getQueryTimingUnits(), null, LogUtils.findCallingClassAndMethod(new Throwable()), mesgBuf.toString()));
                    }
                    if (gatherPerformanceMetrics) this.connection.registerQueryExecutionTime(elapsedTime);
                }
                this.connection.incrementNumberOfPreparedExecutes();
                if (this.profileSQL) {
                    this.eventSink = ProfilerEventHandlerFactory.getInstance(this.connection);
                    this.eventSink.consumeEvent(new ProfilerEvent((byte) 4, "", this.currentCatalog, this.connectionId, this.statementId, -1, System.currentTimeMillis(), mysql.getCurrentTimeNanosOrMillis() - begin, mysql.getQueryTimingUnits(), null, LogUtils.findCallingClassAndMethod(new Throwable()), truncateQueryToLog(asSql(true))));
                }
                ...
                if (mysql.shouldIntercept()) {
                    ...
                    if (interceptedResults != null) rs = interceptedResults;
                }
                if (this.profileSQL) {
                    long fetchEndTime = mysql.getCurrentTimeNanosOrMillis();
                    this.eventSink.consumeEvent(new ProfilerEvent((byte) 5, "", this.currentCatalog, this.connection.getId(), getId(), 0, System.currentTimeMillis(), fetchEndTime - queryEndTime, mysql.getQueryTimingUnits(), null, LogUtils.findCallingClassAndMethod(new Throwable()), null));
                }
                ...
                if (!createStreamingResultSet && this.serverNeedsResetBeforeEachExecution) serverResetStatement();
                this.sendTypesToServer = false;
                this.results = rs;
                if (mysql.hadWarnings()) mysql.scanForAndThrowDataTruncation();
                return rs;
            } catch (SQLException sqlEx) {
                if (mysql.shouldIntercept())
    ...
    private void serverPrepare(String sql) throws SQLException {
        synchronized (checkClosed()) {
            MysqlIO mysql = this.connection.getIO();
            if (this.connection.getAutoGenerateTestcaseScript()) dumpPrepareForTestcase();
            try {
                long begin = 0L;
                ...
                if (this.connection.getProfileSql()) begin = System.currentTimeMillis();
                ...
                for (int i = 0; i < this.parameterCount; i++) this.parameterBindings[i] = new BindValue();
                this.connection.incrementNumberOfPrepares();
                if (this.profileSQL)
                    this.eventSink.consumeEvent(new ProfilerEvent((byte) 2, "", this.currentCatalog, this.connectionId, this.statementId, -1, System.currentTimeMillis(), mysql.getCurrentTimeNanosOrMillis() - begin, mysql.getQueryTimingUnits(), null, LogUtils.findCallingClassAndMethod(new Throwable()), truncateQueryToLog(sql)));
                if (this.parameterCount > 0 && this.connection.versionMeetsMinimum(4, 1, 2) && !mysql.isVersion(5, 0, 0)) {
                    ...
                    int j = 0;
                    while (!metaDataPacket.isLastDataPacket() && j < this.parameterCount) {
                ...
                if (this.fieldCount > 0) {
                    ...
                    int j = 0;
                    while (!fieldPacket.isLastDataPacket() && j < this.fieldCount) {
    ...
    public void setNull(int parameterIndex, int sqlType) throws SQLException {
        checkClosed();
        BindValue binding = getBinding(parameterIndex, false);
        if (binding.bufferType == 0) setType(binding, 6);
        binding.value = null;
        binding.isNull = true;
        binding.isLongData = false;
    }
    ...
    protected void setTimeInternal(int parameterIndex, Time x, Calendar targetCalendar, TimeZone tz, boolean rollForward) throws SQLException {
        synchronized (checkClosed()) {
            if (x == null) {
    ...
    protected void setType(BindValue oldValue, int bufferType) throws SQLException {
        synchronized (checkClosed()) {
            if (oldValue.bufferType != bufferType) this.sendTypesToServer = true;
    ...
    private void storeDateTime413AndNewer(Buffer intoBuf, Date dt, int bufferType) throws SQLException {
        synchronized (checkClosed()) {
            ...
            try {
                ...
                byte length = 7;
                if (dt instanceof Timestamp) length = 11;
                ...
                if (length == 11) intoBuf.writeLong((((Timestamp) dt).getNanos() / 1000));
            } finally {
                sessionCalendar.setTime(oldTime);
            }
        }
    ...
    private Calendar getDefaultTzCalendar() throws SQLException {
        synchronized (checkClosed()) {
            if (this.defaultTzCalendar == null) this.defaultTzCalendar = new GregorianCalendar(TimeZone.getDefault());
    ...
    private void storeReader(MysqlIO mysql, int parameterIndex, Buffer packet, Reader inStream) throws SQLException {
        synchronized (checkClosed()) {
            ...
            int maxBytesChar = 2;
            if (clobEncoding != null) if (!clobEncoding.equals("UTF-16")) {
                maxBytesChar = this.connection.getMaxBytesPerChar(clobEncoding);
                if (maxBytesChar == 1) maxBytesChar = 2;
            } else {
                maxBytesChar = 4;
            }
            ...
            try {
                ...
                while ((numRead = inStream.read(buf)) != -1) {
                    readAny = true;
                    byte[] valueAsBytes = StringUtils.getBytes(buf, (SingleByteCharsetConverter) null, clobEncoding, this.connection.getServerCharacterEncoding(), 0, numRead, this.connection.parserKnowsUnicode(), getExceptionInterceptor());
                ...
                if (totalBytesRead != bytesReadAtLastSend) mysql.sendCommand(24, null, packet, true, null, 0);
                if (!readAny) mysql.sendCommand(24, null, packet, true, null, 0);
                ...
                if (this.connection.getAutoClosePStmtStreams() && inStream != null) try {
                    inStream.close();
                } catch (IOException ioEx) {
                }
            }
    ...
    public boolean canRewriteAsMultivalueInsertStatement() throws SQLException {
        synchronized (checkClosed()) {
            if (!canRewriteAsMultiValueInsertAtSqlLevel()) return false;
            ...
            for (int commandIndex = 0; commandIndex < nbrCommands; commandIndex++) {
                ...
                if (!(arg instanceof String)) {
                    ...
                    if (previousBindValues != null) for (int j = 0; j < this.parameterBindings.length; j++) {
                        if ((currentBindValues[j]).bufferType != (previousBindValues[j]).bufferType) return false;
                    }
                }
            }
            return true;
