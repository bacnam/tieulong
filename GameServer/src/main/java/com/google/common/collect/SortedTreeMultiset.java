...
final class SortedTreeMultiset<E>
    ...
    private SortedTreeMultiset(Comparator<? super E> comparator) {
        ...
        this.nodeFactory = new BstNodeFactory<Node>() {
            public SortedTreeMultiset<E>.Node createNode(SortedTreeMultiset<E>.Node source, @Nullable SortedTreeMultiset<E>.Node left, @Nullable SortedTreeMultiset<E>.Node right) {
                return new SortedTreeMultiset.Node(source.getKey(), source.elemOccurrences, left, right);
            }
        };
    ...
    public int count(@Nullable Object element) {
        ...
        try {
            ...
            if (this.range.contains(e)) {
                Node node = (Node) BstOperations.<E, BstNode>seek(comparator(), this.rootReference.get(), e);
    ...
    private int mutate(E e, MultisetModifier modifier) {
        BstMutationRule<E, Node> mutationRule = BstMutationRule.createRule(modifier, BstCountBasedBalancePolicies.singleRebalancePolicy(this.distinctAggregate), this.nodeFactory);
        BstMutationResult<E, Node> mutationResult = (BstMutationResult) BstOperations.mutate(comparator(), (BstMutationRule) mutationRule, this.rootReference.get(), e);
    ...
    public int remove(@Nullable Object element, int occurrences) {
        ...
        try {
            E e = checkElement(element);
            return this.range.contains(e) ? mutate(e, new RemoveModifier(occurrences)) : 0;
        } catch (ClassCastException e) {
            return 0;
        }
    }
    ...
    Iterator<Multiset.Entry<E>> entryIterator() {
        Node root = this.rootReference.get();
        BstInOrderPath<Node> startingPath = BstRangeOps.<E, Node, BstInOrderPath<Node>>furthestPath(this.range, BstSide.LEFT, this.pathFactory, root);
        return iteratorInDirection(startingPath, BstSide.RIGHT);
    }

    Iterator<Multiset.Entry<E>> descendingEntryIterator() {
        Node root = this.rootReference.get();
        BstInOrderPath<Node> startingPath = BstRangeOps.<E, Node, BstInOrderPath<Node>>furthestPath(this.range, BstSide.RIGHT, this.pathFactory, root);
        return iteratorInDirection(startingPath, BstSide.LEFT);
    }
    ...
    private Iterator<Multiset.Entry<E>> iteratorInDirection(@Nullable BstInOrderPath<Node> start, final BstSide direction) {
        final Iterator<BstInOrderPath<Node>> pathIterator = new AbstractLinkedIterator<BstInOrderPath<Node>>(start) {
            protected BstInOrderPath<SortedTreeMultiset<E>.Node> computeNext(BstInOrderPath<SortedTreeMultiset<E>.Node> previous) {
                if (!previous.hasNext(direction)) return null;
                BstInOrderPath<SortedTreeMultiset<E>.Node> next = previous.next(direction);
                return SortedTreeMultiset.this.range.contains(((SortedTreeMultiset.Node) next.getTip()).getKey()) ? next : null;
            }
        };
        return (Iterator) new Iterator<Multiset.Entry<Multiset.Entry<E>>>() {
            ...
            public Multiset.Entry<E> next() {
                BstInOrderPath<SortedTreeMultiset<E>.Node> path = pathIterator.next();
                return Multisets.immutableEntry(this.toRemove = ((SortedTreeMultiset.Node) path.getTip()).getKey(), (path.getTip()).elemOccurrences);
            }
    ...
    public void clear() {
        Node root = this.rootReference.get();
        Node cleared = BstRangeOps.<E, Node>minusRange(this.range, BstCountBasedBalancePolicies.fullRebalancePolicy(this.distinctAggregate), this.nodeFactory, root);
        if (!this.rootReference.compareAndSet(root, cleared)) throw new ConcurrentModificationException();
    }

    public SortedMultiset<E> headMultiset(E upperBound, BoundType boundType) {
        Preconditions.checkNotNull(upperBound);
        return new SortedTreeMultiset(this.range.intersect(GeneralRange.upTo(this.comparator, upperBound, boundType)), this.rootReference);
    }

    public SortedMultiset<E> tailMultiset(E lowerBound, BoundType boundType) {
        Preconditions.checkNotNull(lowerBound);
        return new SortedTreeMultiset(this.range.intersect(GeneralRange.downTo(this.comparator, lowerBound, boundType)), this.rootReference);
    }
    ...
    private abstract class MultisetModifier implements BstModifier<E, Node> {
        ...
        @Nullable
        public BstModificationResult<SortedTreeMultiset<E>.Node> modify(E key, @Nullable SortedTreeMultiset<E>.Node originalEntry) {
            ...
            if (oldCount == 0) {
                return BstModificationResult.rebalancingChange(null, new SortedTreeMultiset.Node(key, newCount));
            }
            return BstModificationResult.rebuildingChange(originalEntry, new SortedTreeMultiset.Node(key, newCount));
        }
